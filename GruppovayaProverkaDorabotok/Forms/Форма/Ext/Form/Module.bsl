...
Процедура ОбъектыРасширенний(Команда)
	...
	ПараметрыУстановки 	= ПараметрыУстановкиОбъектов();
	ВидОбъектов			= ВидыОбъектовТестирования().Расширенний;
	
	ПараметрыУстановки.ВидОбъектов = ВидОбъектов;
	
	Оповещение = Новый ОписаниеОповещения("ОбъектыДляТестированияВыборЗавершение", ЭтотОбъект, ПараметрыУстановки);
	
	ПоказатьВопрос(Оповещение, "Выбрать только объекты расширенний для тестирования?
	|Нет - дополнить объектами расширенний",
		РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да, "Выберите вариант установки объектов");

КонецПроцедуры

&НаКлиенте
Процедура ОбъектыИзмененные(Команда)
	
	ПараметрыУстановки 	= ПараметрыУстановкиОбъектов();
	ВидОбъектов			= ВидыОбъектовТестирования().Измененные;
	
	ПараметрыУстановки.ВидОбъектов = ВидОбъектов;
	
	Оповещение = Новый ОписаниеОповещения("ОбъектыДляТестированияВыборЗавершение", ЭтотОбъект, ПараметрыУстановки);
	
	ПоказатьВопрос(Оповещение, "Выбрать только измененные объекты для тестирования?
	|Нет - дополнить измененными объектами",
		РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да, "Выберите вариант установки объектов");
		
КонецПроцедуры

&НаКлиенте
Процедура ОбъектыДляТестированияВыборЗавершение(Результат, ДопПараметры) Экспорт

	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДопПараметры.ТолькоВыбранныйВид = (Результат = КодВозвратаДиалога.Да);
	
	Формы_ПроставитьЗначенияПодчиненных(Формы_ДеревоОбъектов, ДопПараметры);
	Отчеты_ПроставитьЗначениеВоВсеСтроки(Отчеты_ДеревоОбъектов, ДопПараметры);
	Печать_ПроставитьЗначение(ДопПараметры);
	Документы_ПроставитьЗначенияПодчиненных(Документы_ДеревоОбъектов, ДопПараметры);
	Справочники_ПроставитьЗначенияПодчиненных(Справочники_ДеревоОбъектов, ДопПараметры);
	
	АктуализироватьВсеФлажки();
	
КонецПроцедуры

&НаКлиенте
Процедура АктуализироватьВсеФлажки()
...
Процедура Формы_УстановитьФлажки(Команда)
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		...
		Для Каждого Индекс Из ВыделенныеСтроки Цикл
			Формы_ПроставитьЗначенияПодчиненных(Формы_ДеревоОбъектов.НайтиПоИдентификатору(Индекс), ПараметрыУстановки);
		КонецЦикла;
		
	Иначе
		Формы_ПроставитьЗначенияПодчиненных(Формы_ДеревоОбъектов, ПараметрыУстановки);
	КонецЕсли;
...
Процедура Формы_СнятьФлажки(Команда)
	...
	ПараметрыУстановки	= ПараметрыУстановкиОбъектов();
	ПараметрыУстановки.Тестировать = Ложь;
...
Процедура Отчеты_ПроставитьВсеФлажки(Команда)
	...
	Отчеты_ПроставитьЗначениеВоВсеСтроки(Отчеты_ДеревоОбъектов, ПараметрыУстановки);
...
Процедура Документы_УстановитьФлажки(Команда)
	...
	Документы_ПроставитьЗначенияПодчиненных(Документы_ДеревоОбъектов, ПараметрыУстановки);
...
Процедура Справочники_УстановитьФлажки(Команда)
	...
	Справочники_ПроставитьЗначенияПодчиненных(Справочники_ДеревоОбъектов, ПараметрыУстановки);
...
&НаКлиентеНаСервереБезКонтекста
Функция ВидыОбъектовТестирования()
	
	ВидыОбъектов = Новый Структура;
	ВидыОбъектов.Вставить("Все"			, "Все");
	ВидыОбъектов.Вставить("Измененные"	, "Измененные");
	ВидыОбъектов.Вставить("Расширенний"	, "Расширенний");
...
&НаКлиентеНаСервереБезКонтекста
Функция ПараметрыУстановкиОбъектов()
	
	ОбъектыВсе = ВидыОбъектовТестирования().Все;
	
	ПараметрыУстановки = Новый Структура;
	ПараметрыУстановки.Вставить("Тестировать"			, Истина);
	ПараметрыУстановки.Вставить("ТолькоВыбранныйВид"	, Ложь);
	ПараметрыУстановки.Вставить("ВидОбъектов" 			, ОбъектыВсе);
...
&НаКлиентеНаСервереБезКонтекста
Процедура Формы_ПроставитьЗначенияПодчиненных(Строка, ПараметрыУстановки)
	
	Для Каждого ПодСтрока Из Строка.ПолучитьЭлементы() Цикл

		УстановитьПризнакТестирования(ПодСтрока, ПараметрыУстановки);  
		
		Если ПодСтрока.ПолучитьЭлементы().Количество() <> 0 Тогда
			Формы_ПроставитьЗначенияПодчиненных(ПодСтрока, ПараметрыУстановки);
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура Документы_ПроставитьЗначенияПодчиненных(Строка, ПараметрыУстановки)
	...
	Для Каждого ПодСтрока Из Строка.ПолучитьЭлементы() Цикл
		...
		Если ПодСтрока.ПолучитьЭлементы().Количество() <> 0 Тогда
			Документы_ПроставитьЗначенияПодчиненных(ПодСтрока, ПараметрыУстановки);
		КонецЕсли;
...
&НаКлиенте
Процедура Печать_ПроставитьЗначение(ПараметрыУстановки)
	...
	Для Каждого СтрокаКорня Из Печать_ДеревоОбъектов.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрокаМетаданного Из СтрокаКорня.ПолучитьЭлементы() Цикл
			...
			Для Каждого СтрокаПечатнойФормы Из СтрокаМетаданного.ПолучитьЭлементы() Цикл
				...
				УстановитьПризнакТестирования(СтрокаПечатнойФормы, ПараметрыУстановки);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьПризнакТестирования(СтрокаОбъекта, ПараметрыУстановки)
	
	Тестировать 		= ПараметрыУстановки.Тестировать;
	ТолькоВыбранныйВид	= ПараметрыУстановки.ТолькоВыбранныйВид; 
	ВидОбъектов			= ПараметрыУстановки.ВидОбъектов; 
	
	ВидРасширенний	= ВидыОбъектовТестирования().Расширенний;
	ВидИзмененные	= ВидыОбъектовТестирования().Измененные;
	
	ОбъектыРасширенийВид =  (ВидОбъектов = ВидРасширенний);
	ОбъектыИзмененныеВид =  (ВидОбъектов = ВидИзмененные);

	ТолькоОбъектыРасширений = (ОбъектыРасширенийВид И ТолькоВыбранныйВид);
	ТолькоОбъектыИзмененные = (ОбъектыИзмененныеВид И ТолькоВыбранныйВид);
	
	Если ОбъектыРасширенийВид Тогда
		
		ТестироватьДругойВид		= ?(ТолькоОбъектыРасширений, НЕ Тестировать, СтрокаОбъекта.Тестировать);
		СтрокаОбъекта.Тестировать	= ?(СтрокаОбъекта.ЕстьИзмененияРасширениями, Тестировать, ТестироватьДругойВид)
		
	ИначеЕсли ОбъектыИзмененныеВид Тогда
		
		ТестироватьДругойВид		= ?(ТолькоОбъектыИзмененные, НЕ Тестировать, СтрокаОбъекта.Тестировать);
		СтрокаОбъекта.Тестировать	= ?(СтрокаОбъекта.ЕстьИзмененияПоддержки, Тестировать, ТестироватьДругойВид);
		
	Иначе
		
		СтрокаОбъекта.Тестировать = Тестировать;
...
Процедура Формы_ДополнитьДеревоФормами(Дерево)
	...
	Дерево.Колонки.Добавить("ЕстьИзмененияПоддержки", Новый ОписаниеТипов("Булево"));
	...
	Для каждого ТекКласс Из Дерево.Строки Цикл
		...
		Для каждого Элем Из ТекКласс.Строки Цикл
			...
			Для каждого ТекФорма Из ФормыМетаОбъекта Цикл
				...
				УстановитьПризнакИзмененияПоддержки(ЭлементДерева, ТекКласс.Имя, МетаОбъект.Имя);
...
Процедура Отчеты_ЗаполнитьТаблицуОтчетов()
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Если ТекущийОтчет <> РезультатЗапроса.Отчет Тогда
			...
			УстановитьПризнакИзмененияПоддержки(СтрокаГруппы, "Отчеты", МетаОбъект.Имя);
			
			ЕстьИзмененияРасширениями	= СтрокаГруппы.ЕстьИзмененияРасширениями;
			ЕстьИзмененияПоддержки		= СтрокаГруппы.ЕстьИзмененияПоддержки;
		...
		СтрокаВарианта.ЕстьИзмененияПоддержки		= ЕстьИзмененияПоддержки;
...
&НаКлиентеНаСервереБезКонтекста
Процедура Отчеты_ПроставитьЗначениеВоВсеСтроки(Узел, ПараметрыУстановки)
	
	Для Каждого Строка Из Узел.ПолучитьЭлементы() Цикл
		
		УстановитьПризнакТестирования(Строка, ПараметрыУстановки);
		Отчеты_ПроставитьЗначениеВоВсеСтроки(Строка, ПараметрыУстановки);
...
Процедура Печать_ЗаполнитьТаблицуПечатныхФорм()
	...
	Для Каждого Источник Из ИсточникиКомандПечати Цикл
		...
		УстановитьПризнакИзмененияПоддержки(СтрокаОбъектаМетаданных, КлассИсточника, Источник.Имя);
		...
		Для Каждого КомандаПечати Из КомандыПечати Цикл
			...
			СтрокаКоманды.ЕстьИзмененияПоддержки 	= СтрокаОбъектаМетаданных.ЕстьИзмененияПоддержки;
...
Процедура Документы_СформироватьДеревоМетаданных()
	...
	НовоеДерево.Колонки.Добавить("ЕстьИзмененияПоддержки",		Новый ОписаниеТипов("Булево"));
	...
	Для Каждого СтрокаДокумент Из НовоеДерево.Строки[0].Строки Цикл
		...
		УстановитьПризнакИзмененияПоддержки(СтрокаДокумент, СтрокаДокумент.Родитель.Имя, МетаОбъект.Имя);
...
Процедура Справочники_СформироватьДеревоМетаданных()
	...
	КоллекцииОбъектовМетаданных.Колонки.Добавить("ЕстьИзмененияПоддержки");
	...
	Для Каждого СтрокаКоллекции Из КоллекцииОбъектовМетаданных Цикл
		...
		Для Каждого ОбъектМетаданных Из Метаданные[СтрокаКоллекции.Имя] Цикл
			...
			УстановитьПризнакИзмененияПоддержки(СтрокаДереваОМ, СтрокаКоллекции.Имя, ОбъектМетаданных.Имя);
			
			Если СтрокаДереваОМ.ЕстьИзмененияПоддержки Тогда
		...
		Если ЕстьИзменения Тогда   
			СтрокаКоллекции.ЕстьИзмененияПоддержки	= Истина;
			СтрокаДерева.ЕстьИзмененияПоддержки		= Истина;
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура Справочники_ПроставитьЗначенияПодчиненных(Строка, ПараметрыУстановки)
	...
	Для Каждого ПодСтрока Из Строка.ПолучитьЭлементы() Цикл
		...
		Если ПодСтрока.ПолучитьЭлементы().Количество() <> 0 Тогда
			Справочники_ПроставитьЗначенияПодчиненных(ПодСтрока, ПараметрыУстановки);
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура АктуализироватьРодителяПоддержки(Строка)
	
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Строка, "Родитель") Тогда
		Возврат;
	КонецЕсли;
	...
	ЕстьИзменения = Ложь;
	Для Каждого Подчиненная Из СтрокаРодитель.Строки Цикл
		
		Если Подчиненная.ЕстьИзмененияПоддержки Тогда
			ЕстьИзменения = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	СтрокаРодитель.ЕстьИзмененияПоддержки = ЕстьИзменения;
	
	АктуализироватьРодителяПоддержки(СтрокаРодитель);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьПризнакИзмененияПоддержки(СтрокаОбъекта, КлассМетаданных, ОбъектМетаданных)
	
	Отбор			= Новый Структура("Класс, Объект", КлассМетаданных, ОбъектМетаданных);
	СтрокиПоддержки = ДанныеПоддержки.НайтиСтроки(Отбор);
		
	СтрокаОбъекта.ЕстьИзмененияПоддержки = ЗначениеЗаполнено(СтрокиПоддержки);
	
	АктуализироватьРодителяПоддержки(СтрокаОбъекта);
